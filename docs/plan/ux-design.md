...
ただし v2 では `useMake10` にギブアップ関連の状態管理を追加し、v3 では難易度レベル状態・レベル別出題ロジック・レベルアップ判定を追加する。既存のゲームロジック（正解判定、スコア計算、式パース）には触れない。
...
flowchart TD
    ...
    C1 --> D1[空欄をタップして選択肢から選ぶ]
    D1 --> E1{選択結果}
    E1 -->|正解| F[正解オーバーレイ + 祝福演出]
    E1 -->|不正解| G1[不正解フィードバック: 選択肢をシェイク]
...
### 2.2 超かんたんモードの詳細フロー（v3 新規）

```mermaid
flowchart TD
    A[穴埋め問題を表示] --> B[正解式に空欄が1つある状態]
    B --> C[空欄スロットがパルスアニメーションで誘導]
    C --> D[ユーザーが空欄スロットをタップ]
    D --> E[選択肢ボタン 3〜4 個が下部に表示]
    E --> F{ユーザーが選択肢を選ぶ}
    F -->|正解の選択肢| G[空欄に正解を埋めて式を完成]
    G --> H[正解オーバーレイ + 祝福演出]
    F -->|不正解の選択肢| I[選択した選択肢をシェイク + 赤くフラッシュ]
    I --> J[空欄スロットに戻る]
    J --> E
...
ポイント:
- 空欄は1つだけ（演算子1つまたは数字1つ）
- 選択肢は常に表示されている（スロットをタップして表示するのではなく、最初から見える）
- 不正解時はスコア変動なし、同じ問題で再挑戦
- 「こたえを みる」ボタンは非表示（超かんたんでは不要。選択肢から必ず正解を選べるため）

### 2.3 かんたんモードの詳細フロー（v3 新規）

```mermaid
flowchart TD
    A[部分組み立て問題を表示] --> B[式に2つの数字が配置済み]
...
### 2.8 初回ガイドフロー（v3 新規: S9）

```mermaid
flowchart TD
    A[初回起動: 星 0] --> B[超かんたんモードの1問目]
    B --> C[空欄スロットに指差しアニメーション表示]
    C --> D[ユーザーが空欄をタップ or 選択肢をタップ]
    D --> E[ガイドが消える]
    E --> F[2問目]
...
ポイント:
- テキスト説明はなし（視覚的な誘導のみ）
- 1問目: 空欄スロットの上に指差し（👆）アニメーション + パルスハイライト
...
アプリは単一画面構成。ルーティングやページ遷移は存在しない。画面内の領域を 5 つのゾーン + 背景レイヤー + オーバーレイ群に分割する。v3 ではレベルに応じてゾーン内のコンポーネントが切り替わる。

### 3.1 画面レイアウト（縦向き 375px 幅基準、v3 更新）
...
+------------------------------------------+
|                                          |
|  [Choice Buttons Zone]                   |  ← NumberPad/OperatorPad の代わり
|                                          |
|  ┌──────────┐  ┌──────────┐             |
|  │    +     │  │    ×     │             |
|  └──────────┘  └──────────┘             |
...
- 空欄位置は BlankSlot コンポーネントで表示
- 式の末尾に「= 10」を常に表示

```
例: 正解式が「3+2+1+4」で「+」(1つ目)が空欄の場合
表示: 3 [☐] 2 + 1 + 4 = 10
```

| 状態 | 表示内容 |
|------|---------|
| 初期 | 式テンプレート + 空欄スロット（パルスアニメーション） |
| 選択後（正解） | 空欄に正解が埋まり、式全体が完成。文字がポップアニメーション |
| 選択後（不正解） | 空欄に不正解が一瞬表示 → シェイクして消える |

テキスト: Nunito, 28px（ふつうの 32px よりやや小さく。式全体 + "= 10" が収まるように）

BlankSlot の仕様:
- サイズ: テキストサイズに合わせた inline-block
- 背景: amber-100
- 枠線: border-2 border-dashed border-amber-400
- 角丸: rounded-lg
- パルスアニメーション: border の色が amber-400 → amber-200 → amber-400 と 1.5s サイクルで脈動
- タップ可能な見た目（cursor: pointer は不要だが、視覚的に「ここを操作する」と分かる）
...
### 3.7 Choice Buttons Zone（v3 新規）

用途: 超かんたんモードで、空欄に入る選択肢を表示する

NumberPad と OperatorPad の位置を置き換えて表示する。
...
### 3.9 Give Up Confirmation Dialog（v2 新規）
...
### 3.10 Answer Reveal Overlay（v2 新規）
...
### 3.11 Feedback Overlay（v1 から継続、v2 更新）
...
- テキスト（空時）: 「?」を表示, text-amber-400, font-bold
- アニメーション: `animate-blank-pulse`（border-color が amber-400 → amber-200 を 1.5s で繰り返す）
- 填められた時: border-solid, bg-amber-100, テキストは選択された値を表示
- 正解時: bg-emerald-100, border-emerald-400, テキスト emerald-700
- 不正解時: bg-red-100, border-red-400, テキスト red-500, animate-choice-shake
...
レベル 1: 超かんたん:
1. 穴埋め式が表示される（空欄 + 選択肢）
2. 選択肢ボタンをタップ → 正解ならオーバーレイ、不正解ならシェイク
3. 正解オーバーレイタップ → レベルアップ判定 → 次の問題

レベル 2: かんたん:
1. 部分組み立て式が表示される（2数字配置済み + 残り入力）
2. 残り数字 / 演算子 / 括弧を入力
3. = タップ → 判定 → オーバーレイ
...
| C7: 超かんたんで空欄2つ | 3.4 Display / 3.7 ChoiceButtons で拡張可能な設計 |
...
interface Make10State {
  ...
  newLevelName: string;
  // 超かんたんモード用
  blankExpression?: string[];   // トークン配列（例: ['3', '☐', '2', '+', '1', '+', '4']）
  blankIndex?: number;          // 空欄の位置
  blankAnswer?: string;         // 空欄の正解
...
### 11.7 穴埋め問題の生成ロジック（v3 新規）

```typescript
// src/logic/fillInBlank.ts

interface FillInBlankPuzzle {
  tokens: string[];      // 式のトークン配列（数字・演算子を個別に）
  blankIndex: number;    // 空欄の位置（tokens 内のインデックス）
  answer: string;        // 空欄の正解
...
function generateFillInBlank(
  ...
  // 1. solutions から1つ選ぶ（シンプルな式を優先）
  // 2. 式をトークンに分解
  // 3. ランダムに1つのトークンを空欄にする
  // 4. ダミー選択肢を生成
...
### 11.8 部分組み立て問題の生成ロジック（v3 新規）

```typescript
// src/logic/partialAssembly.ts

interface PartialAssemblyPuzzle {
  digits: number[];         // 4つの数字
  prefilledIndices: number[]; // 配置済みの2つの数字のインデックス
  // 配置済み数字は NumberPad 上で used 状態にするだけ
  // 式の初期値は空（ユーザーが自由に組み立てる）
}

function generatePartialAssembly(
  digits: number[],
  solutions: string[]
): PartialAssemblyPuzzle {
  // 1. solutions から1つ選ぶ
  // 2. その式で使われている4つの数字のうち、2つをランダムに選ぶ
  // 3. 選ばれた2つの数字のインデックスを返す
}
```
