...
アプリは単一画面構成。ルーティングやページ遷移は存在しない。v4 でタイトルスクリーンを追加するが、React の条件分岐による全画面オーバーレイとして実装する。画面内の領域を 5 つのゾーン + 背景レイヤー + オーバーレイ群 + タイトルスクリーンに分割する。v3 ではレベルに応じてゾーン内のコンポーネントが切り替わる。
